...
import gov.nist.hit.ds.testEngine.context.RetContext;
...
import gov.nist.hit.ds.xdsException.XdsConfigurationException;
import gov.nist.hit.ds.xdsException.XdsException;
import gov.nist.hit.ds.xdsException.XdsIOException;
import gov.nist.hit.ds.xdsException.XdsInternalException;
import gov.nist.hit.ds.xdsException.XdsPreparsedException;
import gov.nist.hit.ds.xdsException.XdsWSException;
...
public class RetrieveB {
	...
	OMElement build_request(RetContext rc) {
		...
		OMElement rdsr = XmlUtil.om_factory.createOMElement(new QName(	ns, "RetrieveDocumentSetRequest"));
		for (String uid : rc.getRequestInfo().keySet()) {
			RetInfo ri = rc.getRequestInfo().get(uid);

			OMElement dr = XmlUtil.om_factory.createOMElement(new QName(	ns, "DocumentRequest"));
			rdsr.addChild(dr);

			if (ri.getHome() != null && !ri.getHome().equals("")) {
				OMElement hci = XmlUtil.om_factory.createOMElement(new QName(	ns, "HomeCommunityId"));
				hci.setText(ri.getHome());
				dr.addChild(hci);
			}

			OMElement ruid = XmlUtil.om_factory.createOMElement(new QName(	ns, "RepositoryUniqueId"));
			ruid.setText(ri.getRep_uid());
			dr.addChild(ruid);

			OMElement duid = XmlUtil.om_factory.createOMElement(new QName(	ns, "DocumentUniqueId"));
