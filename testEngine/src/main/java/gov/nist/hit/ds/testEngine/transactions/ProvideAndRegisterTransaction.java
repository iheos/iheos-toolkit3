...
import gov.nist.hit.ds.soap.axis2.Swa;
	...
	throws XdsException {
		...
		if (xds_version == BasicTransaction.xds_b) {
			...
			for (String id : document_id_filenames.keySet() ) {
				...
				OMText t = XmlUtil.om_factory.createOMText(dataHandler, true);
				t.setOptimize(use_xop);
				OMElement document = XmlUtil.om_factory.createOMElement("Document", MetadataSupport.xdsB);
