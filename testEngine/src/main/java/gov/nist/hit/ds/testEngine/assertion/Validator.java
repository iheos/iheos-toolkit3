package gov.nist.hit.ds.testEngine.assertion;
...
//		return mkArrayList(testname, "false", errs);
	...
	String properAssocFormatting(String type) {
		return	MetadataSupport.associationTypeWithNamespace(type);
	}

	public OMElement hasAssociation(OMElement source, OMElement target, String type) throws MetadataException {
		String source_id = att_val(source, XmlUtil.id_qname);
		String target_id = att_val(target, XmlUtil.id_qname);
	...
	public OMElement hasAssociationWithOneTarget(OMElement source, List<OMElement> targets, String type) throws MetadataException {
		...
		for (OMElement tgt : targets) {
			String id = att_val(tgt, XmlUtil.id_qname);
			target_ids.add(id);
		}
		String type1 = properAssocFormatting(type);
	...
	public boolean docApproved(OMElement eo) {
		String status = m.stripNamespace(eo.getAttributeValue(MetadataSupport.status_qname));
		if ( status == null || !status.equals("Approved")) {
			err("ExtrinsicObject " + eo.getAttributeValue(XmlUtil.id_qname) + " has status " + status + " instead of 'Approved'");
			return false;
		}
		return true;
	}
	...
	public boolean docDeprecated(OMElement eo) {
		String status = m.stripNamespace(eo.getAttributeValue(MetadataSupport.status_qname));
		if ( status == null || !status.equals("Deprecated")) {
			err("ExtrinsicObject " + eo.getAttributeValue(XmlUtil.id_qname) + " has status " + status + " instead of 'Deprecated'");
			return false;
		}
		return true;
	}
	...
	public boolean folsApproved() throws MetadataException {
		for (OMElement fol : m.getFolders()) {
			...
			if ( status == null || !status.equals("Approved")) {
				err("Folder " + fol.getAttributeValue(XmlUtil.id_qname) + " has status " + status + " instead of 'Approved'");
				return false;
			}
		}
		return true;
	...
	public boolean ssApproved() throws MetadataException {
		...
		if ( status == null || !status.equals("Approved")) {
			err("SubmissionSet " + ss.getAttributeValue(XmlUtil.id_qname) + " has status " + status + " instead of 'Approved'");
			return false;
		}
		return true;
	}
