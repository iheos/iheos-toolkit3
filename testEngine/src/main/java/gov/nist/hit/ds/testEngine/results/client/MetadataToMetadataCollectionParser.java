...
import gov.nist.hit.ds.registryMetadata.client.Association;
import gov.nist.hit.ds.registryMetadata.client.Author;
import gov.nist.hit.ds.registryMetadata.client.DocumentEntry;
import gov.nist.hit.ds.registryMetadata.client.Folder;
import gov.nist.hit.ds.registryMetadata.client.MetadataCollection;
import gov.nist.hit.ds.registryMetadata.client.MetadataObject;
import gov.nist.hit.ds.registryMetadata.client.ObjectRef;
import gov.nist.hit.ds.registryMetadata.client.SubmissionSet;
...
public class MetadataToMetadataCollectionParser {
	...
	public MetadataToMetadataCollectionParser(Metadata m, String label) {
		...
		if (col.docEntries.size() == 0 &&
			...
			for (OMElement ele : m.getObjectRefs()) {
				...
				or.id = ele.getAttributeValue(XmlUtil.id_qname);
	...
	void parse(Association assoc, OMElement ele) {
		...
		assoc.versionX = new OMFormatter(XmlUtil.firstChildWithLocalName(ele, "VersionInfo")).toHtml();
	...
	void parse(SubmissionSet ss, OMElement ele) {
		...
		ss.titleX = new OMFormatter(XmlUtil.firstChildWithLocalName(ele, "Name")).toHtml();
		
		ss.comments = asString(m.getDescriptionValue(ele));
		ss.commentsX = new OMFormatter(XmlUtil.firstChildWithLocalName(ele, "Description")).toHtml();
	...
	void parse(DocumentEntry de, OMElement ele) {
		...
		de.versionX = new OMFormatter(XmlUtil.firstChildWithLocalName(ele, "VersionInfo")).toHtml();
		
		de.status = asString(m.getStatus(ele));
		de.statusX = eoEleStr;
		
		de.title = asString(m.getNameValue(ele));
		de.titleX = new OMFormatter(XmlUtil.firstChildWithLocalName(ele, "Name")).toHtml();
		
		de.comments = asString(m.getDescriptionValue(ele));
		de.commentsX = new OMFormatter(XmlUtil.firstChildWithLocalName(ele, "Description")).toHtml();
	...
	void parse(Folder fol, OMElement ele) {
		...
		fol.versionX = new OMFormatter(XmlUtil.firstChildWithLocalName(ele, "VersionInfo")).toHtml();
		
		fol.status = asString(m.getStatus(ele));
		fol.statusX = folEleStr;
		
		fol.title = asString(m.getNameValue(ele));
		fol.titleX = new OMFormatter(XmlUtil.firstChildWithLocalName(ele, "Name")).toHtml();

		fol.comments = asString(m.getDescriptionValue(ele));
		fol.commentsX = new OMFormatter(XmlUtil.firstChildWithLocalName(ele, "Description")).toHtml();
