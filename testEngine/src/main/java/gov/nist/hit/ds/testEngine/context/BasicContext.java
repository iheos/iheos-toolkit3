...
public class BasicContext  {
	...
	public void dumpContextIntoOutput(OMElement output) {
		OMElement context = XmlUtil.om_factory.createOMElement(new QName("Context"));
		...
		if (atts != null) {
			...
			for (Iterator it=keys.iterator(); it.hasNext(); ) {
				String key = (String) it.next();
				Object value = atts.get(key);
				OMElement att_ele = XmlUtil.om_factory.createOMElement(new QName(key));
				att_ele.addChild(XmlUtil.om_factory.createOMText(value.toString()));
	...
	OMElement build_results_document() {
		OMElement result = XmlUtil.om_factory.createOMElement("TestResults", null);
		return result;
	}
