...
class SimConfigFactory {
    ...
    SimConfig buildSim(String server, String port, String tlsPort, String base, SimId simId, ActorType actorType) {
        log.debug("SimConfigFactory: actorType ${actorType.name}")
        SimConfig actorSimConfig = new SimConfig(actorType)

        EndpointBuilder endpointBuilder = new EndpointBuilder(server, port, tlsPort, base, simId)
