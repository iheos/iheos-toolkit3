 ...
 * requires that the class have a setErrorRecorder method and a run method.
 ...
 *  validator components are just called components. If one of the components run generates a 
 ...
 *  the SimComponent interface.  At runtime, the run(...) method is called after injecting an ErrorRecorder
 ...
 *  analyzes the SimChain it identifies generated outputs and necessary inputs by examining the getters and
 ...
 *  has executed, the SimChainAnalyser is run for each component just before execution of that component.
 ...
 *  component to be run; name - a name to be used in displays; description - a fuller description of the
