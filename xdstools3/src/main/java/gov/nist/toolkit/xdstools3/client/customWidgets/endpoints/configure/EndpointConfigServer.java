...
import gov.nist.toolkit.xdstools3.client.customWidgets.endpoints.select.EndpointType;
...
@Path("/endpointconfig")
public class EndpointConfigServer {
	...
	@Consumes( { MediaType.TEXT_XML })
	@Path("/read")
	public EndpointConfigDSResponse read(EndpointConfigDSRequest request) {
        EndpointConfigDSResponse response = new EndpointConfigDSResponse();
        response.setStatus(DSResponse.STATUS_SUCCESS);
//
