...
//		if (!contains(transbean))
	...
	static ATFactory.TransactionType getTransactionFromCode(String transactionCode) {
		return ATFactory.getTransactionFromCode(transactionCode);
	}
	
	static public String getTransactionName(String transactionCode) {
		ATFactory.TransactionType tt = ATFactory.getTransactionFromCode(transactionCode);
		if (tt == null)
			return "";
		return tt.getName();
	}

	static public List<ATFactory.ActorType> getActorTypes() {
		return Arrays.asList(ATFactory.ActorType.values());
	}
	...
	public boolean hasActor(ATFactory.ActorType actor) {
