...
import gov.nist.hit.ds.ebMetadata.IdParser;
...
import gov.nist.hit.ds.testClient.registry.Validator;
...
public abstract class BasicTransaction {
	...
	public void doRun() throws Exception {
		...
		ATFactory.TransactionType ttype = ATFactory.TransactionType.find(trans);
	...
	protected void parseGatewayEndpoint(String home, boolean isSecure) throws Exception {
		if (endpoint == null || endpoint.equals("")) {
			if (s_ctx.getPlan().getRegistryEndpoint() != null) 
				...
				try {
					endpoint = testConfig.site.getEndpoint(ATFactory.TransactionType.XC_RETRIEVE, isSecure, async);
	...
	protected void parseIGREndpoint(String home, boolean isSecure) throws Exception {
		if (endpoint == null || endpoint.equals("")) {
			if (s_ctx.getPlan().getRegistryEndpoint() != null) 
				...
				try {
					endpoint = testConfig.site.getEndpoint(ATFactory.TransactionType.IG_RETRIEVE, isSecure, async);
//					endpoint = testConfig.site.getIGRetrieve(home, isSecure, async);
	...
	MetadataException, MetadataValidationException {
		...
		try {
			...
			fail ("find_instruction_output failed: " + ExceptionUtil.exception_details(e));
