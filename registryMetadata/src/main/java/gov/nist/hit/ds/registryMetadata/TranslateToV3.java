...
public class TranslateToV3 extends Translate {
	...
	OMElement deep_copy(OMElement from, OMNamespace new_namespace) {
		...
		OMElement to = XmlUtil.om_factory.createOMElement(from.getLocalName(), new_namespace);
	...
	protected void copy_attributes(OMElement from, OMElement to) {
		...
		for (Iterator<?> it=from.getAllAttributes(); it.hasNext();  ) {
			...
			OMAttribute to_a = XmlUtil.om_factory.createOMAttribute(name, namespace, value);
		...
		if (element_name.equals("RegistryPackage")) {
			...
			if (object_type_att == null) {
				object_type_att = XmlUtil.om_factory.createOMAttribute("objectType", null, "urn:oasis:names:tc:ebxml-regrep:ObjectType:RegistryObject:RegistryPackage");
			...
			OMAttribute object_type_att = to.getAttribute(MetadataSupport.object_type_qname);
			if (object_type_att == null) {
				object_type_att = XmlUtil.om_factory.createOMAttribute("objectType", null, "urn:oasis:names:tc:ebxml-regrep:ObjectType:RegistryObject:Association");
				to.addAttribute(object_type_att);
			} else {
			...
			OMAttribute object_type_att = to.getAttribute(MetadataSupport.object_type_qname);
			if (object_type_att == null) {
				object_type_att = XmlUtil.om_factory.createOMAttribute("objectType", null, "urn:oasis:names:tc:ebxml-regrep:ObjectType:RegistryObject:Classification");
				to.addAttribute(object_type_att);
			} else {
			...
			OMAttribute object_type_att = to.getAttribute(MetadataSupport.object_type_qname);
			if (object_type_att == null) {
				object_type_att = XmlUtil.om_factory.createOMAttribute("objectType", null, "urn:oasis:names:tc:ebxml-regrep:ObjectType:RegistryObject:ExternalIdentifier");
				to.addAttribute(object_type_att);
			} else {
