...
public class IdParser {
	...
	public OMElement getApproveObjectsRequest(List<?> uuids) {
		OMElement req = XmlUtil.om_factory.createOMElement("ApproveObjectsRequest", null);
		req.addChild(mk_object_ref_list(uuids));
		return req;
	}

	public OMElement getDeprecateObjectsRequest(List<?> uuids) {
		OMElement req = XmlUtil.om_factory.createOMElement("DeprecateObjectsRequest", null);
		req.addChild(mk_object_ref_list(uuids));
		return req;
	...
	private OMElement mk_object_ref_list(List<?> uuids) {
		OMElement object_ref_list = XmlUtil.om_factory.createOMElement("ObjectRefList", null);
		for (Iterator<?> it=uuids.iterator(); it.hasNext(); ) {
			String uuid = (String) it.next();
			OMAttribute att = XmlUtil.om_factory.createOMAttribute("id", null, uuid);
			OMElement object_ref = XmlUtil.om_factory.createOMElement("ObjectRef", null);
