...
public class SubmissionSetVal extends AbstractRegistryObjectVal  implements TopLevelObjectVal {
    ...
    public void validateSlotsCodedCorrectly(ErrorRecorder er, ValidationContext vc)  {
        if (vc.isXDRMinimal) {
            ...
            validateSlot(model, er, 	"submissionTime", 	   false, 	new DtmFormatValidator(er, "Slot submissionTime",            SubmissionSetModel.table416),  SubmissionSetModel.table416);
            validateSlot(model, er, 	"intendedRecipient",   true, 	new XonXcnXtnFormatValidator(er, "Slot intendedRecipient",      SubmissionSetModel.table416),  SubmissionSetModel.table416);
        }
    }
