...
import gov.nist.hit.ds.metadata.ProcessMetadataInterface;
import gov.nist.hit.ds.metadata.store.*;
...
public class ProcessMetadataForRegister implements ProcessMetadataInterface {
	...
	public void checkUidUniqueness(Metadata m) {
		...
		for (OMElement ele : m.getMajorObjects()) {
            ...
            // TODO: Reconnect this
			Ro ro = null;   //mc.getObjectByUid(uid);
			if (ro != null) {
...
//		}
	...
	public void verifyAssocReferences(Metadata m) {
		for (OMElement assocEle : m.getAssociations()) {
			...
			try {
				...
				er.err(XdsErrorCode.Code.XDSRegistryError, "Error extracting associationType attribute from Association " + m.getId(assocEle)
						, this, null);
				continue;
			}
			if (m.contains(source)) {
				...
				if (ro == null) {
					er.err(XdsErrorCode.Code.XDSRegistryError, "Association " +
	...
	public  void doRPLCDeprecations(Metadata m) {
		for (OMElement assocEle : m.getAssociations()) {
			RegIndex.AssocType atype = RegIndex.getAssocType(m, assocEle);
			if (atype == RegIndex.AssocType.RPLC || atype == RegIndex.AssocType.RPLC_XFRM) {
	...
	public void deprecateDoc(String docId) {
		...
		if (ro == null) {
			er.err(XdsErrorCode.Code.XDSRegistryError, "RPLC failed, replaced DocumentEntry [ " + docId + "] does not exist in registry", this, null);
		} else {
			// Deprecate
			...
			List<Assoc> assocs = mc.assocCollection.getBySourceDestAndType(null, docId, RegIndex.AssocType.XFRM);
			assocs.addAll(mc.assocCollection.getBySourceDestAndType(null, docId, RegIndex.AssocType.APND));
		...
		for (OMElement assocEle : m.getAssociations()) {
			...
			if (atype == RegIndex.AssocType.RPLC || atype == RegIndex.AssocType.RPLC_XFRM) {
				...
				for (Fol f : foldersHoldingTarget) {
					try {
						delta.addAssoc(f.getId(), sourceId, RegIndex.AssocType.HASMEMBER);
						delta.labelFolderUpdated(f, new Hl7Date().now());
					} catch (Exception e) {
