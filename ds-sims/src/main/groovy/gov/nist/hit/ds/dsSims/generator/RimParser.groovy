...
class RimParser {
    ...
    def parseClassifications(xml) {
        def attributes = []
        xml.ClassificationValidator.each { classification ->
            ...
            atts['system'] = classification.SlotValidator.ValueList.Value.text()
    ...
    def parseExternalIdentifiers(xml) {
        def attributes = []
        xml.ExternalIdentifierValidator.each { identifier ->
    ...
    def labelRegistryPackages(xml, classification) {
        ...
        rpClassifications << xml.ClassificationValidator.findAll {
            it.@classificationNode == classification
        }.collect { it.@classifiedObject }
        rpClassifications << xml.RegistryPackage.ClassificationValidator.findAll {
            it.@classificationNode == classification
        }.collect { it.@classifiedObject }
