...
//	transient public ValidationContext vc;
	...
	transient List<RegIndex.OldValueNewValueStatus> statusChanges = null;
	...
	public MetadataCollection mkDelta() {
		...
		delta.statusChanges = new ArrayList<RegIndex.OldValueNewValueStatus>();
		
		return delta;
	}
	
	public void changeAvailabilityStatus(String id, RegIndex.StatusValue oldValue, RegIndex.StatusValue newValue) {
		RegIndex.OldValueNewValueStatus ons = (new RegIndex()).new OldValueNewValueStatus(oldValue, newValue, id);
	...
	public boolean mergeDelta(ErrorRecorder er) {
		...
		for (RegIndex.OldValueNewValueStatus ons : statusChanges) {
			RegIndex.StatusValue oldVal = ons.o;
			String id = ons.id;
			...
			if (obj.getAvailabilityStatus() != oldVal) {
				er.err(XdsErrorCode.Code.XDSRegistryError, "mergeDelta: old status has changed", this, null);
				return false;
			}
		...
		for (RegIndex.OldValueNewValueStatus ons : statusChanges) {
			RegIndex.StatusValue newVal = ons.n;
	...
	public List<Fol> getFoldersContaining(String id) {
		...
		List<Assoc> hasmembers = assocCollection.getBySourceDestAndType(null, id, RegIndex.AssocType.HASMEMBER);
	...
	public void addDocEntryToFolAssoc(DocEntry de, Fol f) throws MetadataException, XdsInternalException, IOException {
		addAssoc(f.getId(), de.getId(), RegIndex.AssocType.HASMEMBER);
	}
	...
	void attachAvailabilityStatus(Metadata m) throws XdsInternalException {
		for (OMElement ele : m.getExtrinsicObjects()) {
			...
			try {
				...
				RegIndex.StatusValue sv = de.getAvailabilityStatus();
		...
		for (OMElement ele : m.getFolders()) {
			...
			try {
				Fol f = folCollection.getById(id);
				RegIndex.StatusValue sv = f.getAvailabilityStatus();
				String statusString = RegIndex.getStatusString(sv);
				ele.addAttribute("status", statusString, null);
			}
			catch (Exception e) {
		...
		for (OMElement ele : m.getSubmissionSets()) {
			...
			try {
				...
				RegIndex.StatusValue sv = s.getAvailabilityStatus();
				String statusString = RegIndex.getStatusString(sv);
				ele.addAttribute("status", statusString, null);
			}
			catch (Exception e) {
		...
		for (OMElement ele : m.getAssociations()) {
			...
			try {
				...
				RegIndex.StatusValue sv = a.getAvailabilityStatus();
				String statusString = RegIndex.getStatusString(sv);
				ele.addAttribute("status", statusString, null);
			}
			catch (Exception e) {
	...
	public void storeMetadata(OMElement ele,  boolean overwriteOk) throws IOException, MetadataException, XdsInternalException {
        ...
        // TODO: needs porting
		File rof = null;
//        regIndex.getSimDb().getRegistryObjectFile(id);
	...
	public void addAssoc(String source, String target, RegIndex.AssocType type) throws MetadataException, XdsInternalException, IOException {
		Assoc a = new Assoc();
		a.from = source;
		a.to = target;
		a.type = type;
