...
public class TranslateToV3 extends Translate {
    ...
    public OMElement translate(OMElement ro2, boolean must_dup) throws XdsInternalException {
        if (isV3Namespace(ro2.getNamespace()) && !must_dup)
            return Util.deep_copy(ro2);
        m = new Metadata();
        return deep_copy(ro2, ebRIMns3);
    }
    ...
    OMElement deep_copy(OMElement from, OMNamespace new_namespace) {
        ...
        OMElement to = om_factory.createOMElement(from.getLocalName(), new_namespace);
    ...
    protected void copy_attributes(OMElement from, OMElement to) {
        ...
        for (Iterator it=from.getAllAttributes(); it.hasNext();  ) {
            ...
            if (name.equals("status"))
                value = m.addNamespace(value, status_type_namespace);
...
//			value = "urn:oasis:names:tc:ebxml-regrep:ObjectType:RegistryObject:" + value;
            OMAttribute to_a = om_factory.createOMAttribute(name, namespace, value);
            to.addAttribute(to_a);
        }
        if (element_name.equals("RegistryPackage")) {
            OMAttribute object_type_att = to.getAttribute(object_type_qname);
            if (object_type_att == null) {
                object_type_att = om_factory.createOMAttribute("objectType", null,
            ...
            OMAttribute object_type_att = to.getAttribute(object_type_qname);
            if (object_type_att == null) {
                object_type_att = om_factory.createOMAttribute("objectType", null, "urn:oasis:names:tc:ebxml-regrep:ObjectType:RegistryObject:Classification");
                to.addAttribute(object_type_att);
            } else {
            ...
            OMAttribute object_type_att = to.getAttribute(object_type_qname);
            if (object_type_att == null) {
                object_type_att = om_factory.createOMAttribute("objectType", null, "urn:oasis:names:tc:ebxml-regrep:ObjectType:RegistryObject:ExternalIdentifier");
                to.addAttribute(object_type_att);
            } else {
