...
//				s.pidAllocateURI = ele.getText().trim();
	...
	public OMElement siteToXML(Site s) {
		OMElement site_ele = XmlUtil.om_factory.createOMElement("site", null);

		OMAttribute site_name_att = XmlUtil.om_factory.createOMAttribute("name", null, s.getName());
		...
		if (s.home != null) {
			OMElement home_ele = XmlUtil.om_factory.createOMElement("home", null);
...
//		}
		...
		for (TransactionBean tb : repos.transactions) {
			OMElement trans_ele = XmlUtil.om_factory.createOMElement("repository", null);
			String name_suffix = "";
		...
		if (s.pidAllocateURI != null) {
			XmlUtil.om_factory.createOMComment(site_ele, "Patient ID allocation service to use with this site");
			OMElement pida_ele = XmlUtil.om_factory.createOMElement("PidAllocateEndpoint", null);
	...
	void addTransactionXML(OMElement site_ele, TransactionBean tb) {
		...
		if (isTransaction)
			trans_ele = XmlUtil.om_factory.createOMElement("transaction", null);
		else
			trans_ele = XmlUtil.om_factory.createOMElement("repository", null);
		String name_suffix = "";
		if (tb.isAsync) name_suffix = ".as";
