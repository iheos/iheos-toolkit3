...
public class TranslateToV3 extends Translate {
    ...
    public OMElement translate(OMElement ro2, boolean must_dup) throws XdsInternalException {
        ...
        m = new Metadata();
        OMElement v3 = deep_copy(ro2, MetadataSupport.ebRIMns3);
        return v3;
    }
    ...
    OMElement deep_copy(OMElement from, OMNamespace new_namespace) {
        ...
        // Att.values() holds the V3 required order of ebRIM attributes.
        for (Att att : Att.values()) {
            String att_name = att.name();
            // walk through the attributes present, processing only the ones that
            // match the current att type out of Att.values() (forcing proper order on the output)
            for (Iterator it=from.getChildElements(); it.hasNext(); ) {
                OMElement child = (OMElement) it.next();
                if (!child.getLocalName().equals(att_name)) continue;  // current att type?
        ...
        if (to.getFirstElement() == null) {
            String text = from.getText().trim();
            if (!text.equals(""))  to.setText(text);
        }
