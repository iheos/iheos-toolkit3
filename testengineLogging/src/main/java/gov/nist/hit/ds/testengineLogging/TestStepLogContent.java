...
import gov.nist.hit.ds.registrysupport.logging.RegistryErrorLog;
...
public class TestStepLogContent  implements Serializable {
	...
	public TestStepLogContent(OMElement root) throws Exception {
		...
		id = root.getAttributeValue(XmlUtil.id_qname);
	...
	private void parseDetails() {
		...
		//for (OMElement ele : XmlUtil.childrenWithLocalName(root, "Detail")) {
	...
	void parseResult() {
		try {
			//			result =  xmlFormat(XmlUtil.firstDecendentWithLocalName(root, "Result").getFirstElement());
			OMElement copy = Util.deep_copy(XmlUtil.firstDecendentWithLocalName(root, "Result").getFirstElement());
			//OMElement resultEle = XmlUtil.firstDecendentWithLocalName(root, "Result").getFirstElement();
