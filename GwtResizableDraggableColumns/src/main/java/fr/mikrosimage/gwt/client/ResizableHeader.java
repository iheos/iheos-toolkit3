...
/**
 * This class was originally from fr.mikrosimage.gwt.client, but was modified to fit another use with IndexedColumn type.
 * Some unused parts were commented off.
 * See comments marked with skb for updated version.
 * @param <T>
 */
...
//    protected final Column<T, ?> column;
    protected final IndexedColumn column; // skb
    ...
    public ResizableHeader(String title, AbstractCellTable<T> table, IndexedColumn column /* Column<T, ?> column */) {
...
//            mover.appendChild(left);
        ...
        private SpanElement createSpanElement(String innerText, String title, String backgroundColor, Cursor cursor, double left){
            ...
            try {
                ...
                style.setFontStyle(Style.FontStyle.NORMAL); // skb
                ...
                style.setFontSize(7, Style.Unit.PT); // skb
...
//            System.out.println("entering onPreviewNativeEvent, eventType is: " + eventType);
...
//                System.out.println("clientX: " + clientX + " header.getAbsoluteLeft():" + header.getAbsoluteLeft() + " MINIMUM_COLUMN_WIDTH:" + MINIMUM_COLUMN_WIDTH);
//                System.out.println(" set columnResized: " + Math.max(clientX - header.getAbsoluteLeft(), MINIMUM_COLUMN_WIDTH));

                try {
                    columnResized(column.getIndex(), Math.max(clientX - header.getAbsoluteLeft(), MINIMUM_COLUMN_WIDTH));
                } catch (Throwable t) {
 ...
 /*
    ...
    */
    ...
    protected void columnResized(int columnIdx, int newWidth) {

//        System.out.println("resizing " + columnIdx);

        table.setColumnWidth(columnIdx, newWidth + "px"); // skb

//        table.setColumnWidth(column, newWidth + "px"); // This is a problem when IndexedColumn type is used
    }

/*
    protected void columnMoved(int fromIndex, int toIndex) {
        table.removeColumn(fromIndex);
        table.insertColumn(toIndex, (Column<T, ?>)this.column, this);
