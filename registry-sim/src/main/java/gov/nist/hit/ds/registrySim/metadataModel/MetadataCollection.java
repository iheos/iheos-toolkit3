...
import gov.nist.hit.ds.registrySim.metadataModel.RegIndex.OldValueNewValueStatus;
import gov.nist.hit.ds.registrySim.store.repository.MetadataRepository;
...
public class MetadataCollection implements Serializable, RegistryValidationInterface {
    ...
    transient private Simulator simulator = null;
    ...
    public boolean mergeDelta(IAssertionGroup er) {
        ...
        for (OldValueNewValueStatus ons : statusChanges) {
            ...
            if (obj == null) {
                er.err(Code.XDSRegistryError, new ErrorContext("mergeDelta: cannot find object " + id, null), this);
                return false;
            }

            if (obj.getAvailabilityStatus() != oldVal) {
                er.err(Code.XDSRegistryError, new ErrorContext("mergeDelta: old status has changed", null), this);
                return false;
            }
    ...
    public void addDocEntryToFolAssoc(DocEntry de, Fol f) throws MetadataException, ToolkitRuntimeException, IOException {
    ...
    public Metadata loadRo(String id) throws MetadataException, ToolkitRuntimeException {
        Ro ro = getRo(id);
        if (ro == null)
            return null;
        File f = ro.getFile();
    ...
    public Metadata loadRawRo(String id) throws MetadataValidationException, MetadataException, ToolkitRuntimeException {
        Ro ro = getRo(id);
        if (ro == null)
            return null;
        File f = ro.getFile();
    ...
    public Metadata loadRo(Collection<String> ids) throws MetadataException, ToolkitRuntimeException {
        Metadata m = new Metadata();

        for (String id : ids) {
            Metadata m2 = loadRo(id);
            if (m2 == null)
                throw new ToolkitRuntimeException("Cannot load metadata for object " + id);
            m.addMetadata(m2);
        }
    ...
    Metadata attachFolderLastUpdateTime(Metadata m) throws ToolkitRuntimeException, MetadataValidationException, MetadataException {
        ...
        for (OMElement ele : m.getFolders()) {
            ...
            try {
                ...
                throw new ToolkitRuntimeException("Internal Error: unable to attach lastUpdateTime to Folder " + id);
            }
        }
        if (updateMade)
            return m.reOrder();
    ...
    void attachAvailabilityStatus(Metadata m) throws ToolkitRuntimeException {
        for (OMElement ele : m.getExtrinsicObjects()) {
            ...
            try {
                ...
                throw new ToolkitRuntimeException("Internal Error: unable to attach availabilityStatus to DocumentEntry " + id);
            }
        }
        ...
        for (OMElement ele : m.getFolders()) {
            ...
            try {
                ...
                throw new ToolkitRuntimeException("Internal Error: unable to attach availabilityStatus to Folder " + id);
            }
        }
        ...
        for (OMElement ele : m.getSubmissionSets()) {
            ...
            try {
                ...
                throw new ToolkitRuntimeException("Internal Error: unable to attach availabilityStatus to SubmissionSet " + id);
            }
        }
        ...
        for (OMElement ele : m.getAssociations()) {
            ...
            try {
                ...
                throw new ToolkitRuntimeException("Internal Error: unable to attach availabilityStatus to Association " + id);
            }
        }
    ...
    public void storeMetadata(OMElement ele) {
        String id = new Metadata().getId(ele);
        Ro ro = getRo(id);
        if (ro == null)
            throw new ToolkitRuntimeException("MetadataCollection#storeMetadata: index corrupted");

        OMElement wrapper = XmlUtil.createElement("LeafRegistryObjectList", MetadataSupport.ebRIMns3);
        wrapper.addChild(ele);

        if (simulator == null) throw new ToolkitRuntimeException("MetadataCollection: simulator is null");
        try {
            new MetadataRepository(simulator).saveObject(
                    new OMFormatter(wrapper).toString().getBytes(),
                    id
            );
        } catch (MetadataCannotBeOverwrittenException e) {
            throw new ToolkitRuntimeException("storeMetadata: ", e);
        }
    }
    ...
    public void addAssoc(String source, String target, AssocType type) throws MetadataException, ToolkitRuntimeException, IOException {
        Assoc a = new Assoc();
        a.from = source;
        a.to = target;
        a.type = type;
