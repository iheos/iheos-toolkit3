...
import gov.nist.hit.ds.registryMetadataValidator.datatype.XonXcnFormat;
import gov.nist.hit.ds.registryMetadataValidator.datatype.XonXcnXtnFormat;
...
public class SubmissionSet extends AbstractRegistryObject implements TopLevelObject {
	...
	public SubmissionSet(Metadata m, OMElement ro) throws ToolkitRuntimeException  {
		super(m, ro);
	}
	...
	public void validateDirectSlotsCodedCorrectly(IAssertionGroup er, ValidationContext vc)  {
		...
		if (vc.isXDRMinimal) {
			for (String slotName : requiredSlotsMinimal) {
				if (getSlot(slotName) == null)
					er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext(identifyingString() + ": Slot " + slotName + " missing", table416), this);
			}
		} else {
	...
	public void validateSlotsLegal(IAssertionGroup er)  {
		...
		for (Slot slot : getSlots()) {
			if ( ! legal_slot_name(slot.getName()))
				er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext(identifyingString() + ": " + slot.getName() + " is not a legal slot name for a SubmissionSet",  table416),  this);
