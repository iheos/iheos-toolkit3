	...
	void validateFolderHasMemberAssoc(ErrorRecorder er, String assocId) {
		...
		if (simpleAssocType(m.getAssocType(assoc)).equals("HasMember")) {
			...
			if (m.isFolder(source)) {
				...
				// is not folder
				er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext(source + " is not a Folder in submission and cannot already be in registry", assocsRef), this);
			...
			if (m.isDocument(target)) {
				...
				er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext(source + " is not a DocumentEntry in submission and cannot already be in registry", assocsRef), this);
				hasmember_error = true;
			}
		} else {
			er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext(assocDescription(assocId) + ": only Folder to DocumentEntry associations can be members of SubmissionSet (linked to SubmissionSet object via HasMember association", assocsRef), this);
			hasmember_error = true;
		}
	}
		...
		for (int i=0; i<assocs.size(); i++) {
			...
			if (a_source.equals(ss_id)) {
				...
				if (target_is_included_is_doc) {
					if ( ! m.hasSlot(assoc, "SubmissionSetStatus")) {
						er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext("Association(" +
								...
								") has sourceObject pointing to SubmissionSet and targetObject pointing to a DocumentEntry but contains no SubmissionSetStatus Slot", "ITI TF-3: 4.1.4.1"), this
						);
					} 
				...
				if ( m.hasSlot(assoc, "SubmissionSetStatus") && !"Reference".equals(m.getSlotValue(assoc, "SubmissionSetStatus", 0)))
					er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext("Association " +
							assoc.getAttributeValue(MetadataSupport.id_qname) +
							" does not have sourceObject pointing to SubmissionSet but contains SubmissionSetStatus Slot with value Original", "ITI TF-3: 4.1.4.1"), this
					);
			}
		}
	}
		...
		for (int i=0; i<assocs.size(); i++) {
			...
			if ( target_is_included_doc ) {
				...
				if (ss_status == null || ss_status.equals("")) {
					er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext("SubmissionSetStatus Slot on Submission Set association has no value", "ITI TF-3: 4.1.4.1"), this);
				} else if (	ss_status.equals("Original")) {
					if ( !containsObject(target)) 
						er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext("SubmissionSetStatus Slot on SubmissionSet association has value 'Original' but the targetObject " + target + " references an object not in the submission", "ITI TF-3: 4.1.4.1"), 
								this);
				} else if (	ss_status.equals("Reference")) {
					if (containsObject(target))
						er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext("SubmissionSetStatus Slot on SubmissionSet association has value 'Reference' but the targetObject " + target + " references an object in the submission", "ITI TF-3: 4.1.4.1"),
								this);
				} else {
					er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext("SubmissionSetStatus Slot on Submission Set association has unrecognized value: " + ss_status, "ITI TF-3: 4.1.4.1"), this);
				}
			} else {
				if (ss_status != null && !ss_status.equals("Reference")) 
					er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext("A SubmissionSet Assocation has the SubmissionSetStatus Slot but the target ExtrinsicObject is not part of the Submission", "ITI TF-3: 4.1.4.1"), this);
	...
	void folder_assocs(ErrorRecorder er, ValidationContext vc)  {
		...
		for (OMElement a : m.getAssociations()) {
			...
			if (m.getAssocTarget(a) == ssId)
				er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext("SubmissionSet may not the be target of an Association", "ITI TF-3: 4.1.4"), this);
		...
		for (OMElement a : non_ss_assocs) {
			...
			for (OMElement a2 : m.getAssociations()) {
				if (m.getAssocSource(a2).equals(ssId) &&
					...
					if (good) {
						er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext("Multiple HasMember Associations link SubmissionSet " + ssId + 
								" and Association\n" + a, "ITI TF-3: 4.1.4"), this);
					} else {
						good = true;
					}
			...
			if (good == false)
				er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext("A HasMember Association is required to link SubmissionSet " + ssId + 
						" and Folder/DocumentEntry Association\n" + a, "ITI TF-3: 4.1.4.2"), this);
		}
		...
		for (int i=0; i<docs.size(); i++) {
			...
			if ( assoc == null) {
				er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext("DocumentEntry(" + 
						...
						") is not linked to the SubmissionSet with a " + assoc_type("HasMember") + " Association", "ITI TF-3: 4.1.4.1"),
						this);
			} else {
				if (!has_sss_slot(assoc)) {
					er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext(assocDescription(assoc) + 
							...
							"SubmissionSetStatus Slot with value Original", "ITI TF-3: 4.1.4.1"), 
							this);
					hasmember_error = true;
	...
	void all_fols_linked_to_ss(ErrorRecorder er, ValidationContext vc) {
		...
		for (int i=0; i<fols.size(); i++) {
			...
			if ( !has_assoc(m.getSubmissionSetId(), assoc_type("HasMember"), m.getId(fol))) {
				er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext("Folder " + m.getId(fol) + " is not linked to the SubmissionSet with a " + assoc_type("HasMember") + " Association", "ITI TF-3: 4.1.4.2"),
						this);
				hasmember_error = true;
			}
	...
	void rplced_doc_not_in_submission(ErrorRecorder er, ValidationContext vc)  {
		...
		for (int i=0; i<assocs.size(); i++) {
			...
			if (MetadataSupport.relationship_associations.contains(type) && ! isReferencedObject(id))
				er.err(XdsErrorCode.Code.XDSRegistryMetadataError, new ErrorContext("DocumentEntry referenced by a relationship style assocation " + MetadataSupport.relationship_associations + 
						" cannot be contained in submission\nThe following objects were found in the submission:" 
						+ getIdsOfReferencedObjects().toString(), "ITI TF-3: 4.1.6.1"), this);
		}
	}
