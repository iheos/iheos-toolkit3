package gov.nist.hit.ds.testkitUtilities;
...
public class TestkitStructure extends TestkitWalker {
	...
	public TestkitStructure() throws IOException {
		testkitDesc = XmlUtil.om_factory.createOMElement("testkit", null);
	...
	public OMElement getTest(String testnum) {
		return XmlUtil.getChild(testkitDesc, "test", testnum);
	}
	...
	public List<String> getSectionNames(String testnum) throws Exception {
		...
		for (OMElement sectionEle : XmlUtil.childrenWithLocalName(testEle, "section")) {
			names.add(sectionEle.getAttributeValue(XmlUtil.id_qname));
		}
		return names;
	}
	...
	public OMElement getSection(String testnum, String sectionId) throws Exception {
		...
		if (sectionId == null)
			return testEle;
		return XmlUtil.getChild(testEle, "section", sectionId);
	}
	...
	public List<String> getStepNames(String testnum) throws Exception {
		...
		for (OMElement stepEle : XmlUtil.childrenWithLocalName(testEle, "step")) {
			names.add(stepEle.getAttributeValue(XmlUtil.id_qname));
		}
		return names;
	}
	...
	public List<String> getStepNames(String testnum, String section) throws Exception {
		...
		List<String> names = new ArrayList<String>();
		for (OMElement sectionEle : XmlUtil.childrenWithLocalName(testEle, "section")) {
			String sectionName = sectionEle.getAttributeValue(XmlUtil.id_qname);
	...
	List<String> getStepIds(File testplan) {
		...
		try {
			...
			for (int i=0; i<nodes.size(); i++) {
				...
				ids.add(testStep.getAttributeValue(XmlUtil.id_qname));
			}
	...
	public void startTestPlan(File testplan) throws Exception {
		...
		OMElement test = XmlUtil.getChild(testkitDesc, "test", testId);
		if (test == null) {
			test = XmlUtil.om_factory.createOMElement("test", null);
		...
		if (sectionId != null) {
			section = XmlUtil.om_factory.createOMElement("section", null);
		...
		for (int i=0; i<stepIds.size(); i++) {
			...
			OMElement step = XmlUtil.om_factory.createOMElement("step", null);
